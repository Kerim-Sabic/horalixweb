...
Start tabs do not prewarm a live WebView in v3.1.0. The shell paints first, and WebView2 is created only when the user navigates or wakes a page. This removes launch-time work that made the app feel slow.
...
- Dev builds expose `window.__HORALIX_PERF__` with startup timing, tab counts, visible tab counts, live WebView count, and active tab status.
...
The bundled rules are generated by `npm run generate:dnr` from the local Horalix blocklist plus explicit playback-safe YouTube allow/block rules.
