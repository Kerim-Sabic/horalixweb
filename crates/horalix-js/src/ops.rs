...
use horalix_dom::{DomTree, NodeData, NodeId};
use horalix_net::{CookieJar, HoralixHttpClient};
...
pub struct HoralixState {
    pub dom: Option<DomTree>,
    pub url: String,
    pub title: String,
    pub blocked_urls: Vec<String>,
    pub cookie_jar: Option<Arc<CookieJar>>,
    pub http_client: Option<Arc<HoralixHttpClient>>,
...
pub type SharedState = Rc<RefCell<HoralixState>>;
...
fn op_dom(state: &OpState, #[string] cmd: String, #[string] arg1: String, #[string] arg2: String) -> String {
    ...
    match cmd.as_str() {
        ...
        "document_doctype" => {
            for cid in dom.children(dom.document()) {
                if let Some(n) = dom.get_node(cid) {
                    if let horalix_dom::NodeData::Doctype { name, public_id, system_id } = &n.data {
        ...
        "set_inner_html" => {
            ...
            if !arg2.is_empty() {
                let fragment = horalix_dom::parse_fragment(&arg2);
...
fn op_console_msg(state: &OpState, #[string] level: &str, #[string] msg: &str) {
    let _ = state;
    match level {
        "warn" => tracing::warn!(target: "horalix::console", "{}", msg),
        "error" => tracing::error!(target: "horalix::console", "{}", msg),
        _ => tracing::info!(target: "horalix::console", "{}", msg),
    }
}
