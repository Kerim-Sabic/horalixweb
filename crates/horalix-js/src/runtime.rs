...
use horalix_dom::DomTree;

use crate::module_loader::HoralixModuleLoader;
use crate::ops::{build_extension, HoralixState};

static SNAPSHOT: &[u8] = include_bytes!(env!("HORALIX_SNAPSHOT_PATH"));
...
pub struct HoralixJsRuntime {
    runtime: JsRuntime,
    state: Rc<RefCell<HoralixState>>,
    object_store: HashMap<String, String>,
    object_counter: u64,
}

impl HoralixJsRuntime {
    pub fn new() -> Self {
        Self::with_base_url("about:blank")
    }

    pub fn with_base_url(base_url: &str) -> Self {
        let state = Rc::new(RefCell::new(HoralixState::new()));
        let state_clone = state.clone();

        let module_loader = Rc::new(HoralixModuleLoader::new(base_url));
        ...
        runtime
            .execute_script(
                "<horalix:init>",
                "globalThis.__horalix_objects = {}; globalThis.__horalix_oid = 0; globalThis.__horalix_init();".to_string(),
            )
    ...
    pub fn set_cookie_jar(&self, jar: std::sync::Arc<horalix_net::CookieJar>) {
        self.state.borrow_mut().cookie_jar = Some(jar);
    }

    pub fn set_http_client(&self, client: std::sync::Arc<horalix_net::HoralixHttpClient>) {
    ...
    pub fn set_user_agent(&mut self, ua: &str) {
        ...
        let _ = self.runtime.execute_script(
            "<set-ua>",
            format!("globalThis.__horalix_ua = '{}';", escaped),
        );
    }
    ...
    pub fn evaluate_for_cdp(
        ...
        let meta_code = format!(
            "(function() {{\n\
                ...
                globalThis.__horalix_objects['{oid}'] = __result;\n\
                return {meta_fn};\n\
            }})()",
            expr = cleaned_expr,
            oid = oid,
        ...
        self.object_store.insert(
            oid.clone(),
            format!("globalThis.__horalix_objects['{}']", oid),
        );
        ...
        if await_promise {
            let code = format!(
                "(async function() {{\n\
                    ...
                    globalThis.__horalix_objects['{oid}'] = __result;\n\
                    globalThis.__horalix_await_meta = {meta_fn};\n\
                }})()",
                setup = setup,
            ...
            let meta_result = self.runtime.execute_script(
                "<readMeta>",
                "globalThis.__horalix_await_meta".to_string(),
            ).map_err(|e| format!("JS error: {}", e))?;
    ...
    pub fn release_object(&mut self, object_id: &str) {
        if self.object_store.remove(object_id).is_some() {
            let code = format!(
                "delete globalThis.__horalix_objects['{}'];",
                object_id,
            );
    ...
    pub fn release_object_group(&mut self) {
        let _ = self.runtime.execute_script(
            "<releaseGroup>",
            "globalThis.__horalix_objects = {};".to_string(),
        );
...
impl Default for HoralixJsRuntime {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    use super::*;
    use horalix_dom::parse_html;

    fn setup_runtime(html: &str) -> HoralixJsRuntime {
        let dom = parse_html(html);
        let rt = HoralixJsRuntime::new();
        rt.set_dom(dom);
    ...
    fn setup_runtime_with_cookies(html: &str) -> (HoralixJsRuntime, std::sync::Arc<horalix_net::CookieJar>) {
        let dom = horalix_dom::parse_html(html);
        let jar = std::sync::Arc::new(horalix_net::CookieJar::new());
        let rt = HoralixJsRuntime::new();
        rt.set_dom(dom);
        rt.set_url("http://example.com/test");
        rt.set_title("Test Page");
