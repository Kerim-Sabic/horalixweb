...
globalThis.onerror = function(msg, src, line, col, error) {
  globalThis.__horalix_errors.push({msg: String(msg), src: String(src||""), line, error: String(error||"")});
};
...
class Element extends Node {
  ...
  focus() { globalThis.__horalix_focused = this; globalThis.__horalix_click_target = this; }
  blur() { if (globalThis.__horalix_focused === this) globalThis.__horalix_focused = null; }
  get value() {
...
class Document extends Node {
  ...
  get activeElement() { return globalThis.__horalix_focused || this.body; }
  get implementation() {
    return {
...
globalThis.navigator = {
  get userAgent() { return globalThis.__horalix_ua || "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/145.0.0.0 Safari/537.36"; },
...
function _installWasmStreamingFallback() {
  ...
  if (WebAssembly.instantiateStreaming && WebAssembly.instantiateStreaming.__horalixFallback) return;
  ...
  fallback.__horalixFallback = true;
...
URL.createObjectURL = function(blob) {
  if (blob && typeof blob.text === 'function') {
    const id = 'blob:horalix/' + Math.random().toString(36).substring(2);
    blob.text().then(text => { globalThis.__blobStore[id] = text; });
    return id;
  }
  return 'blob:horalix/fallback';
};
...
globalThis.__horalix_init = function() {
  ...
  const toHide = Object.keys(globalThis).filter(k =>
    k.startsWith('_') || k.includes('horalix') || k.includes('Horalix')
  );
  for (const p of toHide) {
    try { Object.defineProperty(globalThis, p, { enumerable: false }); } catch(e) {
    }
  }
  delete globalThis.__horalix_init;
};
