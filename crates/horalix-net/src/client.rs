...
fn validate_url(url: &Url) -> Result<(), HoralixNetError> {
    let scheme = url.scheme();
    if scheme != "http" && scheme != "https" && scheme != "file" {
        return Err(HoralixNetError::Network(format!(
...
async fn fetch_file_url(url: &Url) -> Result<Response, HoralixNetError> {
    let path = url
        .to_file_path()
        .map_err(|_| HoralixNetError::Network("Invalid file URL".to_string()))?;
    let body = tokio::fs::read(&path)
        .await
        .map_err(|e| HoralixNetError::Network(format!("Failed to read file: {}", e)))?;
...
;
    ...
    pub async fn fetch(&self, url: &Url) -> Result<Response, HoralixNetError> {
        self.fetch_with_method(Method::GET, url, None).await
    }

    pub async fn post_form(&self, url: &Url, body: &str) -> Result<Response, HoralixNetError> {
    ...
    pub async fn fetch_with_method(
    ...
    ) -> Result<Response, HoralixNetError> {
        validate_url(url)?;
        ...
        for _redirect_count in 0..max_redirects {
            ...
            if let Some(interceptor) = self.interceptor.read().await.as_ref() {
                match interceptor.intercept(&request_info).await {
                    ...
                    InterceptAction::Block => {
                        return Err(HoralixNetError::Blocked(current_url.to_string()));
                    }
            ...
            let resp = req_builder.send().await.map_err(|e| {
                ...
                HoralixNetError::Network(format!("{}: {}", current_url, e))
            })?;
            self.in_flight.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
            ...
            if status.is_redirection() {
                if let Some(location) = resp.headers().get(reqwest::header::LOCATION) {
                    let location_str = location.to_str().map_err(|_| {
                        HoralixNetError::Network("Invalid redirect Location header".into())
                    })?;
                    let next_url = current_url.join(location_str).map_err(|e| {
                        HoralixNetError::Network(format!("Invalid redirect URL: {}", e))
                    })?;
            ...
            let body_bytes = resp.bytes().await.map_err(|e| {
                HoralixNetError::Network(format!("Failed to read body: {}", e))
            })?.to_vec();
        ...
        Err(HoralixNetError::TooManyRedirects(current_url.to_string()))
    }
...
impl Default for HoralixHttpClient {
    fn default() -> Self {
        Self::new()
    }
}
