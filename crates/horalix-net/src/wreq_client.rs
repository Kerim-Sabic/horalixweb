...
#[cfg(feature = "stealth")]
use crate::client::{Response, HoralixNetError};
    ...
    pub async fn fetch(&self, url: &Url) -> Result<Response, HoralixNetError> {
        ...
        for _ in 0..20 {
            ...
            let resp = req.send().await.map_err(|e| {
                ...
                HoralixNetError::Network(format!("{}: {} (source: {:?})", current_url, e, e.source()))
            })?;
            self.in_flight.fetch_sub(1, std::sync::atomic::Ordering::Relaxed);
            ...
            if status.is_redirection() {
                if let Some(location) = resp.headers().get("location") {
                    let location_str = location.to_str().map_err(|_| {
                        HoralixNetError::Network("Invalid redirect Location".into())
                    })?;
        ...
        Err(HoralixNetError::TooManyRedirects(url.to_string()))
    }
