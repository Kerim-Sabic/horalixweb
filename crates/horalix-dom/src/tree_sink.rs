...
pub struct HoralixElemName<'a> {
    _ref: Ref<'a, ()>,
    name: *const QualName,
}

impl<'a> fmt::Debug for HoralixElemName<'a> {
...
impl<'a> ElemName for HoralixElemName<'a> {
...
impl TreeSink for DomTree {
    type Handle = NodeId;
    type Output = Self;
    type ElemName<'a> = HoralixElemName<'a>;
    ...
    fn elem_name<'a>(&'a self, target: &'a NodeId) -> HoralixElemName<'a> {
