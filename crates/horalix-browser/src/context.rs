...
use horalix_net::{CookieJar, HoralixHttpClient, RobotsCache};
...
impl BrowserContext {
    pub fn new(id: String) -> Self {
        ...
        let http_client = Arc::new(HoralixHttpClient::with_cookie_jar(cookie_jar.clone()));
        BrowserContext {
            id,
            cookie_jar,
            http_client,
    ...
    pub fn with_full_options(
        ...
        let cookie_jar = Arc::new(CookieJar::new());
        let mut client = HoralixHttpClient::with_options(
            cookie_jar.clone(),
            proxy_url.as_deref(),
        );
        if stealth {
