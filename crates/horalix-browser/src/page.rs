...
use horalix_dom::{parse_html, DomTree};
use horalix_js::runtime::HoralixJsRuntime;
use horalix_net::{HoralixHttpClient, HoralixNetError, Response};
use url::Url;
...
#[cfg(feature = "stealth")]
use horalix_net::StealthHttpClient;
...
pub struct Page {
    ...
    pub js: Option<HoralixJsRuntime>,
    pub lifecycle: LifecycleState,
    pub http_client: Arc<HoralixHttpClient>,
    ...
    intercept_tx: Option<tokio::sync::mpsc::UnboundedSender<horalix_js::ops::InterceptedRequest>>,
    #[cfg(feature = "stealth")]
...
impl Page {
    ...
    async fn do_fetch(&self, url: &Url) -> Result<Response, HoralixNetError> {
        #[cfg(feature = "stealth")]
    ...
    fn init_js(&mut self) {
        if self.js.is_some() {
            ...
            let _ = js.execute_script("<reset>",
                "_cache.clear(); globalThis.__horalix_objects = {}; globalThis.__horalix_oid = 0; \
        ...
        let mut rt = HoralixJsRuntime::with_base_url(&self.url_string());
        rt.set_url(&self.url_string());
        rt.set_title(&self.title);

        #[cfg(feature = "stealth")]
        if self.stealth_client.is_some() {
            rt.set_user_agent(horalix_net::STEALTH_USER_AGENT);
    ...
    async fn execute_scripts(&mut self) {
        ...
        let mut fetched: std::collections::HashMap<usize, (String, String, horalix_net::Response)> = std::collections::HashMap::new();
        ...
        if !css_sources.is_empty() {
            if let Some(js) = &mut self.js {
                ...
                let escaped = combined_css
                    ...
                    .replace('`', "\\`")
                    .replace("${", "\\${");
                let code = format!("globalThis.__horalix_css = `{}`;", escaped);
    ...
    ) -> horalix_js::runtime::RemoteObjectInfo {
        if let Some(js) = &mut self.js {
    ...
    pub fn set_intercept_tx(&mut self, tx: tokio::sync::mpsc::UnboundedSender<horalix_js::ops::InterceptedRequest>) {
...
impl From<HoralixNetError> for PageError {
    fn from(e: HoralixNetError) -> Self {
        PageError::NetworkError(e.to_string())
    }
}
