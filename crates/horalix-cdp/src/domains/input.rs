        ...
        "dispatchMouseEvent" => {
            ...
            if event_type == "mousePressed" {
                if let Some(page) = ctx.get_session_page_mut(session_id) {
                    let code = format!(
                        "(function() {{\
                            var target = globalThis.__horalix_click_target || document.activeElement || document.body;\
                            if (!target) return;\
                            var evt = new MouseEvent('mousedown', {{bubbles:true,cancelable:true,clientX:{x},clientY:{y},button:{button},detail:{click_count}}});\
                            target.dispatchEvent(evt);\
                            var click = new MouseEvent('click', {{bubbles:true,cancelable:true,clientX:{x},clientY:{y},button:{button},detail:{click_count}}});\
                ...
                if let Some(page) = ctx.get_session_page_mut(session_id) {
                    let code = format!(
                        "(function() {{\
                            var target = globalThis.__horalix_click_target || document.activeElement || document.body;\
                            if (!target) return;\
                            var evt = new MouseEvent('mouseup', {{bubbles:true,cancelable:true,clientX:{x},clientY:{y},button:{button},detail:{click_count}}});\
                            target.dispatchEvent(evt);\
                        }})()",
                        x = x, y = y, button = button, click_count = click_count,
                    );
