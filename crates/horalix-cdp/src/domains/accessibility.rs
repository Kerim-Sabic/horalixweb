...
fn compute_name(dom: &DomTree, node: &horalix_dom::Node) -> Option<String> {
...
fn compute_value(_dom: &DomTree, node: &horalix_dom::Node) -> Option<String> {
...
fn compute_properties(_dom: &DomTree, node: &horalix_dom::Node) -> Vec<Value> {
    if let NodeData::Element { name, attrs, .. } = &node.data {
        let tag = name.local.as_ref();
