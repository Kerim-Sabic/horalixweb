...
pub async fn start_with_full_options(
    ...
    info!("Horalix Web CDP server listening on ws://127.0.0.1:{}", port);
    info!(
...
async fn cdp_processor(
    ...
    let (itx, irx) = mpsc::unbounded_channel::<horalix_js::ops::InterceptedRequest>();
    ctx.intercept_tx = Some(itx);
    let mut intercept_rx: Option<mpsc::UnboundedReceiver<horalix_js::ops::InterceptedRequest>> = Some(irx);
    let mut intercepted_paused: HashMap<String, tokio::sync::oneshot::Sender<horalix_js::ops::InterceptResolution>> = HashMap::new();
...
fn handle_fetch_resolution(
    ...
    _ctx: &mut CdpContext,
    reply_tx: &mpsc::UnboundedSender<String>,
    intercepted_paused: &mut HashMap<String, tokio::sync::oneshot::Sender<horalix_js::ops::InterceptResolution>>,
) {
    if let Ok(req) = serde_json::from_str::<CdpRequest>(text) {
        ...
        if let Some(resolver) = intercepted_paused.remove(request_id) {
            ...
            let resolution = match method {
                "Fetch.continueRequest" => horalix_js::ops::InterceptResolution::Continue {
                ...
                "Fetch.fulfillRequest" => {
                    ...
                    let headers = req.params.get("responseHeaders")
                        ...
                        .unwrap_or_default();
                    horalix_js::ops::InterceptResolution::Fulfill { status, headers, body }
                }
                "Fetch.failRequest" => {
                    let reason = req.params.get("errorReason").and_then(|v| v.as_str()).unwrap_or("Failed").to_string();
                    horalix_js::ops::InterceptResolution::Fail { reason }
                }
                _ => return,
            };
...
async fn process_with_interception(
    ...
    intercept_rx: &mut Option<mpsc::UnboundedReceiver<horalix_js::ops::InterceptedRequest>>,
    intercepted_paused: &mut HashMap<String, tokio::sync::oneshot::Sender<horalix_js::ops::InterceptResolution>>,
) {
    ...
    let wait_until = req.params.get("waitUntil")
        .and_then(|v| {
            if let Some(s) = v.as_str() {
                Some(horalix_browser::WaitUntil::from_str(s))
            } else if let Some(arr) = v.as_array() {
                arr.iter()
                    .filter_map(|item| item.as_str())
                    .map(horalix_browser::WaitUntil::from_str)
                    .max_by_key(|w| match w {
                        horalix_browser::WaitUntil::DomContentLoaded => 0,
                        horalix_browser::WaitUntil::Load => 1,
                        horalix_browser::WaitUntil::NetworkIdle2 => 2,
                        horalix_browser::WaitUntil::NetworkIdle0 => 3,
                    })
            } else {
                None
            }
        })
        .unwrap_or(horalix_browser::WaitUntil::Load);
    ...
    let (nav_done_tx, mut nav_done_rx) = mpsc::channel::<(horalix_browser::Page, Result<(), String>)>(1);
...
async fn handle_http_json(stream: TcpStream, port: u16, endpoint: &str) -> anyhow::Result<()> {
    ...
    let body = match endpoint {
        "version" => serde_json::to_string_pretty(&json!({
            "Browser": "Horalix Web/3.0.0",
            "Protocol-Version": "1.3",
            "User-Agent": "Horalix Web/3.0.0 (Headless Browser)",
            "V8-Version": "N/A",
            "WebKit-Version": "N/A",
