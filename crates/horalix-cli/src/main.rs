...
use horalix_browser::{BrowserContext, Page};
use tokio::io::{AsyncBufReadExt, AsyncWriteExt, BufReader};
use tokio::process::Command as TokioCommand;
use tokio::time::{timeout, Duration};

#[derive(Parser)]
#[command(name = "horalix", about = "Horalix Web - lightweight headless browser tooling for web scraping and automation")]
struct Args {
    ...
    match args.command {
        Some(Command::Serve { port, proxy, user_agent, stealth, workers }) => {
            ...
            if workers > 1 {
                ...
                horalix_cdp::start_with_full_options(port, proxy, stealth, user_agent).await?;
            }
        }
        ...
        None => {
            ...
            horalix_cdp::start_with_options(args.port, args.proxy, false).await?;
        }
    }
...
async fn run_fetch(
    ...
    let wait_condition = horalix_browser::lifecycle::WaitUntil::from_str(wait_until);
...
fn extract_readable_text(dom: &horalix_dom::DomTree, node_id: horalix_dom::NodeId) -> String {
    use horalix_dom::NodeData;
...
async fn run_parallel_scrape(
    ...
    let worker_path = std::env::current_exe()
        .ok()
        .and_then(|p| p.parent().map(preferred_worker_path))
        .unwrap_or_else(|| std::path::PathBuf::from("horalix-worker"));
...
fn preferred_worker_path(bin_dir: &std::path::Path) -> std::path::PathBuf {
    let horalix_worker = bin_dir.join("horalix-worker");
    if horalix_worker.exists() {
        return horalix_worker;
    }

    bin_dir.join("horalix-worker")
}
